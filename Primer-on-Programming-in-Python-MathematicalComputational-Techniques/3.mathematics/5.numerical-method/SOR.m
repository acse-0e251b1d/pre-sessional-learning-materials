...
%decompose matrix A into Diagonal component matrix D, Lower triangular 
% matrix L, and upper triangular matrix U, such that:
%A = D + L + U
%( D + ωL ) x = ωb – [ ωU + ( ω – 1 ) D ] x ( D + ωL ) 
% x = ωb – [ ωU + ( ω – 1 ) D ] x 
% During the iteration process, the left hand side of the equation is 
% solved by using the previous value for x on right hand side. 
